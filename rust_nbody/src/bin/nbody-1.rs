...
// Note 2: the C program takes an argument body bodies[]. 
// While this looks like an array, 
// it’s really just a pointer — it’s equivalent to body *bodies due to array decay. 
// As it’s non-const, the Rust equivalent is *mut body.
// 将c语言的数组，转化为*mut body
// v------------------------------------ Note 1
unsafe fn offset_Momentum(bodies: *mut body) {
// ^                               ^---- Note 2
    // 遍历行星的速度和质量，算出太阳对应维度的速度改变量
    // i为行星的索引，m为速度的维度
...
// time rustc -C opt-level=3 -C target-cpu=native -C codegen-units=1     src/nbody-1.rs -o nbody-1