...
fn advance(bodies: &mut [Body; BODIES_COUNT]) {
    ...
    // I’m using the “freeze” pattern to update a mutable array in-place within the block, 
    // but then assign it to a non-mut binding, preventing accidental further mutation.
    let magnitudes = {
